  ...
  Drupal.behaviors.paragraphs = {
    attach: function(context, settings) {
      ...
      $('.field--widget-entity-reference-paragraphs table.field-multiple-table').each(function(paragraphIndex) {
        ...
        $(once('paragraph-item-once', $this.find('> tbody > tr'))).each(function(paragraphRowIndex) {
            var $row = $(this);
            ...
            if ($row.find(paragraphGuide).find('+ .paragraphs-subform').length) {
              $(once('paragraph-item-toggle-once', $row.find(paragraphGuide).find('> .paragraph-type-title'))).append('<button class="paragraph-item-toggle" data-row-reference="' + paragraphIndex + '-' + paragraphRowIndex + '" aria-label="' + label + '" aria-expanded="' + (label === expandRowText) + '">' + character + '</button>');
            }
          });
        ...
        $(once('paragraph-toggle-once', $this.find('.field-label').first())).append('<button class="paragraph-toggle ' + className + '" data-paragraph-reference="' + paragraphIndex + '">' + togglerText + '</button>');
      });

      /**
       * Complete paragraph toggler
       */
      $(once('paragraph-rows-toggle', '.paragraph-toggle', context)).on('click', function(e) {
        e.preventDefault();
       ...
       */
      $(once('paragraph-row-toggle', '.paragraph-item-toggle', context)).on('click', function(e) {
        e.preventDefault();
